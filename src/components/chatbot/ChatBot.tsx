...
export default function ChatBot() {
  // Extract chat state and functions from our context
  const { 
    messages, 
    conversationId, 
    updateMessages, 
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Chat header */}
      <div className="bg-secondary text-text-dark p-3 flex justify-between items-center">
        <div className="font-semibold">El Molino Support</div>
        <button 
          onClick={() => {
            setMinimized(true);
            // After a brief delay to allow the animation to play, close the chat
            setTimeout(() => setOpen(false), 300);
          }}
          className="text-text-dark hover:text-text-light focus:outline-none"
        >
          ...
          >
            <div 
              className={`inline-block p-3 rounded-lg max-w-[80%] ${
                message.role === 'user' 
                  ? 'bg-sky-blue text-text-dark' // User message styling
      ...
      {/* Message input form */}
      <form onSubmit={handleSubmit} className="border-t p-3 flex bg-white">
        <input
          type="text"
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="Type your message..."
          className="flex-grow border rounded-l-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-sky-blue"
          disabled={isLoading} // Disable during API calls
        />
        <button
          type="submit"
          className="bg-secondary hover:bg-secondary-dark text-text-dark px-4 py-2 rounded-r-lg disabled:opacity-50"
